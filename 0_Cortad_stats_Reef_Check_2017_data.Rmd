...
#define the path for a few directories
Bleaching_data_directory="C:/Users/Shannon/Desktop/Ecoregions"
#Bleaching_data_directory="C:/RobsR/Shannon/Bleaching data"
Cortad_directory="D:/Cortad_2017"
...
#may need to alter the code for variable "Bleaching_days_since_1981123" based on what the format is in the Bleaching_Data file that you read in. May need to write the column headers in the Bleaching_Data file for latitude and longitude as "Latitude.Degrees" and "Longitude.Degrees" so that it works with this code.

#note: scale of cortad is 4 km   https://www.nodc.noaa.gov/sog/cortad/  . reef check coordinates are to the .1 degrees,minutes,seconds... A tenth of a minute of Latitude (or tenth of a nautical mile) is about 600 feet or 185 meters (longitude at the equator) -googled this converstion http://www.coastalnavigation.com/samples/sec_1/1_pages/1_2.htm
...
```{r remove data points that NA values for lat, lon, average bleaching, depth, or date}
...
#after removing rows with NA's we are left with 9215 studies
...
n_lat_steps<-floor((FilledSST_lat_bounds[1,1]-Bleaching_Data$Latitude.Degrees[i])/lat_step+1)
  ...
  #if the grid cell we grabbed is correct
...
#max_index_of_CoRTAD<-dim(WindSpeed_time_bounds)[2]+1
for (i in 1:number_of_surveys)
{
...
#TSA<-nc_open("cortadv5_TSA.nc", write=FALSE, readunlim=TRUE, verbose=FALSE)
...
```{r for each lat,lon,date Reef Check survey corresponding Cortad grid cell, grab the corresponding Cortad variables}
for (i in 1:number_of_surveys)
{
...
```{r for any lat,lon,date grid cell that is still NA, expand to the next grid cells}
#double-checking NA's for temperature
for (i in 1:number_of_surveys)
{
  if(!is.na(Bleaching_cortad_lon_cell[i]))
  {
...
csv_Title<-paste(StudyTitle, "_with_cortad_variables.csv", sep="")
write.csv(Bleaching_Data_with_cortad_variables, file = csv_Title)
```
